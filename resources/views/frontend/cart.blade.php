                ...
                @if ( isset($products) && count($products) > 0)
                    @foreach ($products as $key => $product)
                        @php
                            ...
                            isset($carts[$product->id]) ? ($priceEnd = $price * $carts[$product->id]) : ($priceEnd = 0);
                            $total += $priceEnd;
                        @endphp
                        @if (isset($carts[$product->id]))
                            <li class="header-cart-item flex-w flex-t m-b-12">
                                ...
                                <div class="header-cart-item-txt p-t-8">
                                    ...
                                    <span class="header-cart-item-info">Số lượng :
                                        {{ isset($carts[$product->id]) ? $carts[$product->id] : 0 }}</span>
